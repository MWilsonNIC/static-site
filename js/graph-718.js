...
function applyStructuredLayout(nodes, links) {
    ...
    categoryNodes.forEach((categoryNode, categoryIndex) => {
        ...
        subcategoryNodes.forEach((subcategoryNode, subIndex) => {
            ...
            const compactCategory = ["Amphibians", "Reptiles"].includes(
                categoryNode.name
            );
            ...
            const patientPoints = getSpeciesPoints(
                patientNodes.length,
                speciesRadius
            );
...
function getPanelMeta(node) {
    if (node.group === "Patient") {
        return `Count: ${node.count || 0}`;
    }

    return `Species count: ${node.speciesCount || 0}`;
}

function getPanelDescription(node) {
    if (node.group === "Category") {
        return node.description || "";
    }

    return node.description || DUMMY_DESCRIPTION;
}

function getPanelContext(node) {
    if (node.group === "Category") return "";
...
function updateInfoPanel(node) {
    ...
    const scientificName = node.scientificName
        ? `<p class="mb-1 text-sm italic text-stone-500">${escapeHtml(node.scientificName)}</p>`
        : "";

    const context = getPanelContext(node)
        ? `<p class="mb-1 text-sm text-stone-500">${escapeHtml(getPanelContext(node))}</p>`
        : "";

    const meta = `<p class="mb-3 text-sm font-medium text-stone-700">${escapeHtml(getPanelMeta(node))}</p>`;

    const descriptionText = getPanelDescription(node);
    const description = descriptionText
        ? `<p class="text-sm leading-relaxed text-stone-600">${escapeHtml(descriptionText)}</p>`
        : "";

    const showImage = node.group === "Patient";
    const imageMarkup = showImage
        ? `
            <img
              src="${escapeHtml(node.img || `${DUMMY_IMAGE}&text=${encodeURIComponent(node.name)}`)}"
    ...
    infoPanel.innerHTML = `
        ...
        <h3 class="mb-1">${escapeHtml(node.name)}</h3>
        ${scientificName}
        ${context}
        ${meta}
        ${description}
    ...
    categories.forEach(group => {
        ...
        group.subcategories.forEach(subcategory => {
            ...
            subcategory.animals.forEach(animal => {
                ...
                // const nodeSize = Math.max(0.5, Math.sqrt(animal.count) * 0.7);
                const nodeSize = Math.max(0.4, animal.count * 0.12);
    ...
    if (wrapper && tooltip) {
        ...
        wrapper.addEventListener("mouseleave", () => {
            ...
            if (window.innerWidth >= 1024) {
                updateInfoPanel(selectedNode || null);
            }
        });
    }
