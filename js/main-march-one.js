...
//     });
// })();
// Responsive tabs (tabs on md+, accordion on mobile)
(() => {
    ...
    roots.forEach((root) => {
        // Important: only use triggers that belong to THIS data-rt root
        const triggers = Array.from(root.querySelectorAll("[data-rt-trigger]")).filter(
            (btn) => btn.closest("[data-rt]") === root,
        );
        ...
        const panels = triggers
            .map((btn) => {
                ...
                // Extra safety for nested components
        ...
        function applyMode() {
            ...
            if (toTabs) {
                ...
                // keep current active panel when switching to mobile
        ...
        // Avoid duplicate listeners if this file is ever re-run
