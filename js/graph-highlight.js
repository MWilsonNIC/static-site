...
function initGraph(data) {
    ...
    const nodeById = new Map(nodes.map(node => [node.id, node]));
    const speciesToSubcategory = new Map();
    const subcategoryToCategory = new Map();
    const subcategoryToSpecies = new Map();
    const categoryToSubcategories = new Map();
    ...
    links.forEach(link => {
        ...
        if (sourceNode.group === "Category" && targetNode.group === "Subcategory") {
            subcategoryToCategory.set(targetId, sourceId);

            if (!categoryToSubcategories.has(sourceId)) {
                categoryToSubcategories.set(sourceId, []);
            }
            categoryToSubcategories.get(sourceId).push(targetId);
        }

        if (sourceNode.group === "Subcategory" && targetNode.group === "Patient") {
            speciesToSubcategory.set(targetId, sourceId);

            if (!subcategoryToSpecies.has(sourceId)) {
                subcategoryToSpecies.set(sourceId, []);
            }
            subcategoryToSpecies.get(sourceId).push(targetId);
        }
    });
    ...
    function getActiveNode() {
        const node = hoveredNode || selectedNode || null;

        if (!node || node.group === "Category") {
            return null;
        }
    ...
    function buildHighlightState(node) {
        clearHighlight();

        if (!node || node.group === "Root") return;

        if (node.group === "Patient") {
            const subcategoryId = speciesToSubcategory.get(node.id);
            const categoryId = subcategoryId
                ? subcategoryToCategory.get(subcategoryId)
                : null;

            highlightedNodeIds.add(node.id);

            if (subcategoryId) {
                highlightedNodeIds.add(subcategoryId);
                highlightedLinkIds.add(getLinkKey(subcategoryId, node.id));
            }

            if (categoryId && subcategoryId) {
                highlightedNodeIds.add(categoryId);
                highlightedLinkIds.add(getLinkKey(categoryId, subcategoryId));
            }
        ...
        if (node.group === "Subcategory") {
            const categoryId = subcategoryToCategory.get(node.id);
            const speciesIds = subcategoryToSpecies.get(node.id) || [];

            highlightedNodeIds.add(node.id);

            if (categoryId) {
                highlightedNodeIds.add(categoryId);
                highlightedLinkIds.add(getLinkKey(categoryId, node.id));
            }

            speciesIds.forEach(speciesId => {
                highlightedNodeIds.add(speciesId);
                highlightedLinkIds.add(getLinkKey(node.id, speciesId));
            });
        ...
        if (node.group === "Category") {
            ...
            const subcategoryIds = categoryToSubcategories.get(node.id) || [];

            subcategoryIds.forEach(subcategoryId => {
                highlightedNodeIds.add(subcategoryId);
                highlightedLinkIds.add(getLinkKey(node.id, subcategoryId));

                const speciesIds = subcategoryToSpecies.get(subcategoryId) || [];
                speciesIds.forEach(speciesId => {
                    highlightedNodeIds.add(speciesId);
                    highlightedLinkIds.add(getLinkKey(subcategoryId, speciesId));
                });
            });
        }
    }

    function getDisplayNodeColor(node) {
        if (node.group === "Root") return node.color;
        ...
        return highlightedNodeIds.has(node.id)
            ? node.color
            : mixColors(node.color, colors.dimTarget, 0.76);
    }

    function getDisplayLinkColor(link) {
        const sourceId =
            typeof link.source === "object" ? link.source.id : link.source;
        const targetId =
            typeof link.target === "object" ? link.target.id : link.target;

        if (sourceId === "Root") return "rgba(0,0,0,0)";

        const activeNode = getActiveNode();
        if (!activeNode) return link.color;

        return highlightedLinkIds.has(getLinkKey(sourceId, targetId))
            ? link.color
            : mixColors(link.color, colors.dimTarget, 0.82);
    }

    function getDisplayLinkWidth(link) {
        const baseWidth = getLinkWidthForDisplay(link, container);
        const sourceId =
            typeof link.source === "object" ? link.source.id : link.source;
        const targetId =
            typeof link.target === "object" ? link.target.id : link.target;
        ...
        return highlightedLinkIds.has(getLinkKey(sourceId, targetId))
            ? baseWidth * 1.25
            : Math.max(baseWidth * 0.35, 0.12);
    }

    function refreshHighlight() {
        buildHighlightState(getActiveNode());
    ...
    graph = ForceGraph3D()(container)
        ...
        .nodeLabel(() => "")
        .nodeColor(node => getDisplayNodeColor(node))
        .nodeVal(node => node.val * getResponsiveNodeScale(container))
        .nodeResolution(18)
        .nodeOpacity(1)
        .linkOpacity(0.45)
        .linkColor(link => getDisplayLinkColor(link))
        .linkWidth(link => getDisplayLinkWidth(link))
        ...
        .onNodeHover(node => {
            ...
            if (window.innerWidth >= 1024) {
                updateInfoPanel(node || selectedNode || null);
            }
    ...
    requestAnimationFrame(() => {
        ...
        graph.linkWidth(link => getDisplayLinkWidth(link));
        refreshHighlight();
